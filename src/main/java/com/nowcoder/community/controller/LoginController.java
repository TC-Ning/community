...
//        session.setAttribute("kaptcha", text);
    ...
    public String login(Model model, HttpServletResponse response, String username, String password,
                        String code, boolean rememberme, @CookieValue(value = "kaptchaOwner", required = false) String kaptchaOwner) {
        // 检查验证码
    ...
    public String getForgetCode(String email, HttpSession session) {
        if(StringUtils.isBlank(email)) {
            return "{\"code\":\"1\",\"msg\":\"邮箱不能为空！\"}";
        }
        String code = userService.sendForgetCode(email);
        session.setAttribute("verifyCode", code);
        return "{\"code\":\"0\",\"msg\":\"\"}";
    }

    @PostMapping("/forget/password")
    public String resetPassword(Model model, HttpSession session, String email, String verifyCode, String password) {
